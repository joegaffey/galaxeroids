class InfoScreen extends PIXI.Sprite {
  constructor() {    
    ...
    this.width = 800;
    this.height = 600;
    new FontFace('PressStart', 'url(./assets/PressStart2P.ttf)').load().then((font) => {
    // new FontFace('PressStart', 'url(./assets/press-start-2p.woff2)').load().then((font) => {
  ...
  addText() {
    let style = new PIXI.TextStyle({
      fontFamily: 'PressStart, Verdana, sans-serif',
      fontSize: 22,
      fill: ['#FF4444']
    });
    ...
    style = new PIXI.TextStyle({
      fontFamily: 'PressStart, Courier, sans-serif',
      fontSize: 14,
      lineHeight: 20,
      fill: ['#888888']
    });
...
Start/Pause  Enter     Start     
  ...
  setScore() {
    this.score.text = `SCORE ${ app.score }`;
  }
